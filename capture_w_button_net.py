...
try:
    while True:
       ...
       # ==============================================================================
        # --- LOGIKA FILTER 2 TOMBOL FISIK (OK & MAKAN) ---
        # ==============================================================================
        ...
        if java_serial and java_serial.in_waiting > 0:
            ...
            if len(perintah_dari_java) > 0:
                ...
                # Skenario 4: Instruksi Jalan dari Stockfish (Contoh: b8c6 1 0 1)
                else:
                    print(f"\n[MESIN CATUR] Instruksi Diterima dari Java: {perintah_dari_java}")
                    if esp_serial: esp_serial.write((perintah_dari_java + '\n').encode('utf-8'))
                    
                    # Parsing data langkah robot
                    ...
                    if len(parts) >= 1:
                        ...
                        if len(move) >= 4 and baseline_state is not None:
                            ...
                            if from_sq in baseline_state and to_sq in baseline_state:
                                ...
                                print(f"[POKA-YOKE] Memori Otomatis Diperbarui: Robot {from_sq} -> {to_sq}")
                        
                        # Sinkronisasi otomatis status warna robot dari Java
                        if len(parts) >= 4:
                            robot_color_flag = parts[3]

                    # Simulasi balas OK ke Java agar GUI terbuka kembali
                    time.sleep(2)
                    java_serial.write("OK".encode('utf-8')) 
                    print("[INFO] Giliran pemain kembali.")
